...
import { CustomOrderPreview_uni71101 } from "~/routes/($locale).products.uni-711-01.custom/components/custom-order-preview"
...
import { CustomOrderOption_uni71101 } from "~/lib/custom-order/options/uni-711-01"

export default function Route() {
  const data = useLoaderData<typeof loader>()

  const [state, dispatch] = useReducer(customOrderReducer, {
    construction: customOrderConstruction.norwegianSpecial,
  ...
  const option = new CustomOrderOption_uni71101(state)
...
export async function loader(props: LoaderFunctionArgs) {
  const handle = "uni-711-01-custom"
